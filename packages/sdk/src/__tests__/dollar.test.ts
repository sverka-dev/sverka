...
describe("$ tagged template", () => {
  ...
  it("rejects unsupported interpolation values", () => {
    expect(() => $`echo ${{ foo: "bar" } as unknown as string}`).toThrowError(
...
describe("StepBuilder", () => {
  ...
  it("dependsOn() adds control dependencies", () => {
    const proj = new Project("test");
    const pipeline = new Pipeline(proj, "ci");
    const step = $`npm test`.dependsOn(["build"]).build(pipeline, "test");
  ...
  it("timeout() sets the timeout", () => {
    const proj = new Project("test");
    const pipeline = new Pipeline(proj, "ci");
    const step = $`npm test`.timeout(5000).build(pipeline, "test");
