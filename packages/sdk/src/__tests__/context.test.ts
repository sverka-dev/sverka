...
describe("context namespaces", () => {
  ...
  it("secrets.NPM_TOKEN → ContextRef(secrets, NPM_TOKEN)", () => {
    expect(secrets.NPM_TOKEN).toEqual({
      kind: "context",
      namespace: "secrets",
      field: "NPM_TOKEN",
    });
  ...
  it("dynamic env access via bracket notation", () => {
    expect(env["MY_CUSTOM_VAR"]).toEqual({
      kind: "context",
      namespace: "env",
      field: "MY_CUSTOM_VAR",
    });
