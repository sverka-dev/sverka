...
| §9.2         | SDK as layer _over_ Construct API                               | Inverted — SDK over flat `core`                  |
...
| `planner` (discovery)     | **Partial reuse**     | Project discovery (detect PMs, propose checks) reusable. Plan _synthesis_ rebuilds (produces Definition Graph, not flat Plan). |
