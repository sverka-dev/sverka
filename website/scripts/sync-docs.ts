...
async function cleanDocsRoot() {
  // Generated by `bun run docs:api` (TypeDoc) — not owned by this sync.
  const keepDirs = new Set(["api"]);
...
async function writeSidebarConfig(entries: FileEntry[]) {
  ...
  // `docs:api` before sync-docs, so include the section when it exists.
  if (await exists(path.join(docsRoot, "api"))) {
    sidebar.push({
      label: "API Reference",
      collapsed: true,
