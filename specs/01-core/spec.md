...
export { workflow } from "./composables/workflow.js";
...
2. **Composition**
   ...
   - `when("false", op)` records the operation with `status: "skipped"`.
   - `matrix({ node: ["20", "24"] }, op)` produces two nodes with distinct ids.
   - `after()` and `with()` merge specs correctly.

3. **DAG validation**
   - A cycle in `dependsOn` raises `CompositionError` with node ids in context.
   - Duplicate resolved ids raise `CompositionError`.
